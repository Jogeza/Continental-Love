  ...
  return (
    <main className="bg-[var(--warm-ivory)]">
      <section className="relative isolate min-h-[560px] overflow-hidden bg-[#11100d] text-white lg:min-h-[640px]">
        ...
        <div className="relative mx-auto flex min-h-[560px] max-w-7xl items-end px-5 py-14 sm:px-8 lg:min-h-[640px] lg:px-12 lg:py-20">
          <div className="max-w-2xl">
            ...
            <a href="#collection" className="mt-8 inline-flex min-h-11 items-center bg-[var(--heritage-gold)] px-7 text-xs font-semibold uppercase tracking-[.14em] text-[#11100d] transition-colors hover:bg-[#c99a3b]">Explore the collection</a>
          </div>
        </div>
      </section>
      ...
      <section id="collection" className="mx-auto max-w-7xl scroll-mt-8 px-5 pb-20 sm:px-8 lg:px-12">
        <div className="mb-9 border-b border-black/12 pb-5"><p className="text-[11px] font-semibold uppercase tracking-[.2em] text-[var(--heritage-gold)]">Shop {config.handle}</p><h2 className="mt-2 text-3xl">The collection</h2></div>
      ...
      <section className="bg-[#171611] text-white">
        <div className="mx-auto grid max-w-7xl lg:grid-cols-2">
          ...
          <div className="flex flex-col justify-center px-6 py-16 sm:px-12 lg:px-16"><p className="text-[11px] font-semibold uppercase tracking-[.24em] text-[var(--heritage-gold)]">{config.storyEyebrow}</p><h2 className="mt-5 text-4xl leading-tight sm:text-5xl">{config.storyTitle}</h2><p className="mt-6 max-w-xl text-sm leading-7 text-white/70 sm:text-base">{config.storyCopy}</p><Link href={config.storyHref} className="mt-6 inline-flex min-h-11 w-fit items-center text-xs font-semibold uppercase tracking-[.16em] underline decoration-[var(--heritage-gold)] underline-offset-8">{config.storyLinkLabel} →</Link></div>
        </div>
      </section>
